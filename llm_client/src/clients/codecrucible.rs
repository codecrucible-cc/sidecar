...
pub struct CodeCrucibleClient {
    client: reqwest_middleware::ClientWithMiddleware,
    api_base: String,
}

#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
struct CodeCrucibleMessage {
    role: String,
    content: String,
    cache_point: bool,
}
...
#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
struct CodeCrucibleRequestOptions {
    temperature: f32,
}

#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
struct CodeCrucibleRequest {
    messages: Vec<CodeCrucibleMessage>,
    options: CodeCrucibleRequestOptions,
    model: String,
...
#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
pub struct CodeCrucibleRequestPrompt {
    prompt: String,
    temperature: f32,
...
#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
pub struct CodeCrucibleChoice {
    pub text: String,
}

#[derive(serde::Serialize, serde::Deserialize, Debug, Clone)]
pub struct CodeCruciblePromptResponse {
    choices: Vec<CodeCrucibleChoice>,
}

impl CodeCrucibleRequestPrompt {
...
impl CodeCrucibleClient {
    ...
    // returns codecrucible access token
    fn access_token(&self, api_key: LLMProviderAPIKeys) -> Result<String, LLMClientError> {
        match api_key {
            LLMProviderAPIKeys::CodeCrucible(api_key) => Ok(api_key.access_token),
    ...
    pub async fn stream_completion_with_tool(
        ...
        match response.status() {
            reqwest::StatusCode::UNAUTHORIZED => {
                error!("Unauthorized access to Codecrucible API");
                return Err(LLMClientError::UnauthorizedAccess);
            }
            reqwest::StatusCode::TOO_MANY_REQUESTS => {
                error!("Rate limit exceeded for Codecrucible API");
...
#[async_trait]
impl LLMClient for CodeCrucibleClient {
    ...
    async fn stream_prompt_completion(
        ...
        let code_crucible_request = CodeCrucibleRequestPrompt::from_string_request(request)?;
        let model = code_crucible_request.model.to_owned();
        let response = self
            .client
            .post(endpoint)
            .json(&code_crucible_request)
            .send()
            .await?;
        ...
        while let Some(event) = response_stream.next().await {
            match event {
                Ok(event) => {
                    ...
                    let response = serde_json::from_str::<CodeCruciblePromptResponse>(&event.data);
                    match response {
                        Ok(response) => {
                            let delta = response
                                .choices
