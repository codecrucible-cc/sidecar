...
async fn connect(data_dir: &str) -> Result<SqlitePool> {
    let url = format!("sqlite://{data_dir}/codecrucible.data?mode=rwc");
...
fn reset(data_dir: &str) -> Result<()> {
    let db_path = Path::new(data_dir).join("codecrucible.data");
    let bk_path = db_path.with_extension("codecrucible.bk");
