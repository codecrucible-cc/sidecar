...
impl ScratchPadAgent {
    ...
    pub async fn human_message_agentic(
        ...
        let ccde_rules = human_agentic_request.ccde_rules().clone();
        ...
        | ToolOutput::BigSearch(important_symbols) = tool_output
        {
            ...
            for (symbol, _) in symbols.iter() {
                initial_symbol_search_information.push(InitialSearchSymbolInformation::new(
                    ...
                    // TODO(codecrucible): umm.. how can we have a file path for a symbol
            ...
            .map(
                |(
                    ...
                    {
                        // TODO(codecrucible+caching): We should be sending the edit request directly
            ...
            // TODO(codecrucible): We should play with the parallelism over here
            .buffered(1)
            .collect::<Vec<_>>()
            .await;
        }
