...
use super::session::{CcdeAgentMode, Session};
...
{iteration_request}"#
    ...
    pub async fn tool_use_agentic(
        ...
        let tool_agent = ToolUseAgent::new(
            ...
            // os can be passed over here safely since we can assume the anvil is running
            // close to the vscode server
            // we should ideally get this information from the vscode-server side setting
            std::env::consts::OS.to_owned(),
            shell.to_owned(),
            ToolUseAgentProperties::new(running_in_editor, repo_name, ccde_rules),
        )
