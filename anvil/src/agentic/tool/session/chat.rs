...
impl SessionChatClient {
    ...
    fn system_message(&self, context: &SessionChatClientRequest) -> String {
        ...
        let ccde_rules = match context.ccde_rules.as_ref() {
            Some(ccde_rules) => {
                format!(
                    r#"
- The user has taken the time to write out a set of guidelines called Ccde Rules for you to follow and always respect, given below:
{ccde_rules}"#
                )
            }
            None => "".to_owned(),
...
{project_labels_context}{ccde_rules}
...
impl Tool for SessionChatClient {
    async fn invoke(&self, input: ToolInput) -> Result<ToolOutput, ToolError> {
        ...
        // so now chat will be routed through codecrucible provider
