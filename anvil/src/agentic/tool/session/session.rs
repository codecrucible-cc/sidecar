...
// TODO(codecrucible): The user is probably going to add more context over here as they
    ...
    fn agentic_edit(
        ...
        Self {
            ...
            exchange_type: ExchangeType::Edit(ExchangeTypeEdit {
                ...
                user_context,
                exchange_type: CcdeEditMode::Agentic,
            }),
            exchange_state: ExchangeState::UserMessage,
            is_compressed: false,
            is_hidden: false,
    ...
    fn anchored_edit(
        ...
        Self {
            ...
            exchange_type: ExchangeType::Edit(ExchangeTypeEdit {
                ...
                user_context,
                exchange_type: CcdeEditMode::Anchored,
            }),
            exchange_state: ExchangeState::UserMessage,
            is_compressed: false,
            is_hidden: false,
...
{}
    ...
    pub async fn react_to_feedback(
        ...
        let exchange_to_react = self
            ...
            .map(|exchange| match &exchange.exchange_type {
                ExchangeType::AgentChat(agentic_chat) => match agentic_chat.reply {
                    ...
                    ExchangeReplyAgent::Edit(_) => Some(CcdeAgentMode::Edit),
                    ExchangeReplyAgent::Plan(_) => Some(CcdeAgentMode::Plan),
        ...
        if accepted {
            if matches!(exchange_to_react, Some(CcdeAgentMode::Plan)) {
            ...
            if matches!(exchange_to_react, Some(CcdeAgentMode::Edit)) {
                let _ = message_properties
                    .ui_sender()
    ...
    pub async fn reply_to_last_exchange(
        ...
        match exchange_reply {
            CcdeAgentMode::Chat => {
                self.chat_reply(tool_box, parent_exchange_id, ccde_rules, message_properties)
                    .await
            }
            CcdeAgentMode::Plan => {
                todo!("plan branch")
    ...
    pub async fn perform_plan_generation(
        ...
        let cloned_ccde_rules = ccde_rules.clone();
        ...
        let edit_task = tokio::spawn(async move {
            ...
            let ccde_rules = cloned_ccde_rules;
                ...
                if let Some(file_to_edit) = step.file_to_edit() {
                    ...
                    let _ = hub_sender.send(SymbolEventMessage::new(
                        SymbolEventRequest::simple_edit_request(
                            ...
                            .set_ccde_rules(ccde_rules.clone()),
                            ToolProperties::new(),
                        ),
