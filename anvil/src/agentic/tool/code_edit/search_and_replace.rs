...
pub struct SearchAndReplaceEditingRequest {
    ...
    // ccde rules which are passed everywhere to the LLM
    ccde_rules: Option<String>,
    should_stream: bool,
}
...
impl SearchAndReplaceEditing {
    ...
    fn system_message(&self, context: &SearchAndReplaceEditingRequest) -> String {
        let ccde_rules = context.ccde_rules.clone();
        let ccde_rules = match ccde_rules {
            Some(ccde_rules) => {
...
impl Tool for SearchAndReplaceEditing {
    async fn invoke(&self, input: ToolInput) -> Result<ToolOutput, ToolError> {
            ...
            while let edits_response = edits_receiver.recv().await {
                ...
                match edits_response {
                    Some(EditDelta::EditLockAcquire(sender)) => {
                        ...
                        // TODO(codecrucible): here we want to first get the lock for the file and re-read
        ...
        match run_with_cancellation(cancellation_token.clone(), llm_response).await {
            Some(Ok(Ok(response))) => {
                // if the self apply tag is enabled this implies that the anvil
            ...
            match self.search_block_status.clone() {
                ...
                SearchBlockStatus::BlockStart => {
                    ...
                    if answer_line_at_index == divider {
                        ...
                        match range {
                            ...
                            None => {
                                // TODO(codecrucible): release the lock immediately
                ...
                SearchBlockStatus::BlockFound((_, block_range)) => {
                        ...
                        .any(|updated_trace| *updated_trace == answer_line_at_index)
                    {
                        ...
                        // TODO(codecrucible): release the lock over here which we were holding on to
...
mod tests {
    ...
    pub fn get_function_return(&self) -> Option<&LLMClientMessageFunctionReturn> {
        ...
        let edits = r#"/Users/skcd/test_repo/anvil/llm_client/src/clients/types.rs
```rust
<<<<<<< SEARCH
    pub fn concat(self, other: Self) -> Self {
        // We are going to concatenate the 2 llm client messages togehter, at this moment
...
```"#;
    ...
    pub fn fs_file_path(&self) -> &str {
        ...
        let edits = r#"/Users/zi/codecrucible/testing/anvil/anvil/src/agentic/symbol/events/edit.rs
```rust
<<<<<<< SEARCH
impl SymbolToEdit {
    pub fn new(
...
/Users/zi/codecrucible/testing/anvil/anvil/src/agentic/symbol/events/edit.rs
```rust
<<<<<<< SEARCH
    pub fn is_outline(&self) -> bool {
        self.outline
...
/Users/skcd/scratch/anvil/anvil/src/bin/something.rs
```rust
<<<<<<< SEARCH
=======
fn main() {
