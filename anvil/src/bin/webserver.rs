...
use axum::Extension;
use clap::Parser;
use anvil::application::{application::Application, config::configuration::Configuration};
...
pub async fn start(app: Application) -> anyhow::Result<()> {
    ...
    let public_routes = Router::new()
        .route("/config", get(anvil::webserver::config::get))
        .route(
            "/reach_the_devs",
            get(anvil::webserver::config::reach_the_devs),
        )
        .route("/version", get(anvil::webserver::config::version))
    ...
    api = api.route("/health", get(anvil::webserver::health::health));
...
fn agentic_router() -> Router {
    ...
    Router::new()
        .route(
            "/probe_request_stop",
            post(anvil::webserver::agentic::probe_request_stop),
        )
        .route(
            "/code_sculpting_followup",
            post(anvil::webserver::agentic::code_sculpting),
        )
        .route(
            "/code_sculpting_heal",
            post(anvil::webserver::agentic::code_sculpting_heal),
        )
        // route for push events coming from the editor
        .route(
            "/diagnostics",
            post(anvil::webserver::agentic::push_diagnostics),
        )
        // SWE bench route
        // This route is for software engineering benchmarking
        .route("/swe_bench", get(anvil::webserver::agentic::swe_bench))
        .route(
            "/agent_session_chat",
            post(anvil::webserver::agentic::agent_session_chat),
        )
        .route(
            "/agent_session_edit_anchored",
            post(anvil::webserver::agentic::agent_session_edit_anchored),
        )
        .route(
            "/agent_session_edit_agentic",
            post(anvil::webserver::agentic::agent_session_edit_agentic),
        )
        .route(
            "/agent_session_plan",
            post(anvil::webserver::agentic::agent_session_plan),
        )
        .route(
            "/agent_session_plan_iterate",
            post(anvil::webserver::agentic::agent_session_plan_iterate),
        )
        .route(
            "/agent_tool_use",
            post(anvil::webserver::agentic::agent_tool_use),
        )
        .route(
            "/verify_model_config",
            post(anvil::webserver::agentic::verify_model_config),
        )
        .route(
            "/cancel_running_event",
            post(anvil::webserver::agentic::cancel_running_exchange),
        )
        .route(
            "/user_feedback_on_exchange",
            post(anvil::webserver::agentic::user_feedback_on_exchange),
        )
        .route(
            "/user_handle_session_undo",
            post(anvil::webserver::agentic::handle_session_undo),
        )
        .route(
            "/move_to_checkpoint",
            post(anvil::webserver::agentic::move_to_checkpoint),
        )
        .route(
            "/get_mcts_data",
            post(anvil::webserver::agentic::get_mcts_data),
        )
}
...
fn tree_sitter_router() -> Router {
    ...
    Router::new()
        ...
        .route(
            "/valid_xml",
            post(anvil::webserver::tree_sitter::check_valid_xml),
        )
}
