...
struct A {
    ...
    fn test_documentation_parsing_rust_another() {
        let source_code = "/// Returns the default user ID as a `String`.\n///\n/// The default user ID is set to \"codecrucible\".\nfn default_user_id() -> String {\n    \"codecrucible\".to_owned()\n}";
        let tree_sitter_parsing = TSLanguageParsing::init();
        let documentation = tree_sitter_parsing.parse_documentation(source_code, "rust");
        assert_eq!(
            documentation,
...
"#;
    ...
    fn test_rust_error_checking() {
        let source_code = r#"use anvil::{embedder::embedder::Embedder, embedder::embedder::LocalEmbedder};
use std::env;
...
fn agent_router() -> Router {
    ...
    Router::new()
        .route(
            "/search_agent",
            get(anvil::webserver::agent::search_agent),
        )
        .route(
            "/hybrid_search",
            get(anvil::webserver::agent::hybrid_search),
        )
        .route("/explain", get(anvil::webserver::agent::explain))
        .route(
            "/followup_chat",
            post(anvil::webserver::agent::followup_chat),
        )
}
        "#;
        let language = "rust";
...
export type ICcdeProbeProgress =
    | ICcdeChatMarkdownContent
    | ICcdeProbeBreakdownContent
    | ICcdeProbeGoToDefinition
    | ICcdeProbeTextEdit
    | ICcdeProbeOpenFile
    | ICcdeProbeRepoMapGeneration
    | ICcdeProbeLongContextSearch
    | ICcdeProbeInitialSymbols"#;
        let tree_sitter_parsing = TSLanguageParsing::init();
        let ts_language_config = tree_sitter_parsing
            .for_lang("typescript")
            .expect("language config to be present");
