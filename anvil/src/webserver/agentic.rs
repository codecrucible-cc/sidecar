...
use llm_client::provider::{
    CodeCrucibleLLMTypes, CodecrucibleAccessToken, LLMProvider, LLMProviderAPIKeys,
};
use serde_json::json;
...
use crate::agentic::tool::session::session::CcdeAgentMode;
...
pub async fn user_feedback_on_exchange(
    ...
    let llm_provider = model_configuration
        ...
        .unwrap_or(LLMProperties::new(
            ...
            LLMProvider::CodeCrucible(CodeCrucibleLLMTypes::new()),
            LLMProviderAPIKeys::CodeCrucible(CodecrucibleAccessToken::new(access_token.to_owned())),
        ));
    ...
    let done_stream = futures::stream::once(async move {
        Ok(sse::Event::default()
            .json_data(json!(
                {"done": "[CODECRUCIBLE_DONE]".to_owned(),
...
pub async fn agent_session_chat(
    ...
    // bring this back later
    let agent_mode = CcdeAgentMode::Chat;
...
pub async fn agent_session_edit_anchored(
    ...
    // bring this back later
    let _agent_mode = CcdeAgentMode::Edit;
...
pub async fn verify_model_config(
    ...
    // match llm_provider {
    //     Some(llm_provider) => {
    //         if llm_provider.provider().is_codecrucible() {
    //             return Ok(Json(AgenticVerifyModelConfigResponse {
    //                 valid: true,
    //                 error: None,
    //             }));
...
pub async fn agent_tool_use(
    ...
    let reasoning = if whoami::username() == "skcd".to_owned()
        ...
        || std::env::var("ANVIL_ENABLE_REASONING").map_or(false, |v| !v.is_empty())
    {
        reasoning
    } else {
