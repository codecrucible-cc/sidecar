...
/// This fixes the model when we are using the anvil provider
/// we need to change the model to the one that the anvil provider
pub fn fix_model_for_anvil_provider(
    provider: &LLMProviderAPIKeys,
    fast_model: LLMType,
) -> LLMType {
    if matches!(provider, LLMProviderAPIKeys::CodeCrucible(_)) {
