...
use crate::inline_completion::helpers::{fix_model_for_anvil_provider, get_indentation_string};
use crate::{
...
impl FillInMiddleCompletionAgent {
    ...
    > {
        ...
        // If we are using the codecrucible provider, use the only model compatible with the codecrucible
        // provider.
        let fast_model = match model_config.provider_for_fast_model() {
            Some(provider) => {
                fix_model_for_anvil_provider(provider, model_config.fast_model.clone())
            }
        ...
        // TODO(skcd): We should replace this with making the call from the anvil
...
fn immediate_terminating_condition(
    ...
    //     {
    //         dbg!("anvil.inline_autocomplete.stop.next_line_similarity");
    //         return TerminationCondition::Immediate;
    //     }
    // }
